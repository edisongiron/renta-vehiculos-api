...
from models.alquiler import Alquiler, EstadoAlquiler, AlquilerCreate, AlquilerDevolucion, AlquilerResponse, AlquilerDetallado, CalcularCosto, CostoResponse
...
class Alquileres:
    ...
    def obtener_alquileres(
        estado: Optional[EstadoAlquiler] = Query(None, description="Filtrar por estado del alquiler"),
    ...
    def obtener_alquiler_detallado(alquiler_id: int) -> AlquilerDetallado:
        ...
        cliente = next((c for c in clientes_db if c.id == alquiler.cliente_id), None)
        vehiculo = next((v for v in vehiculos_db if v.id == alquiler.vehiculo_id), None)

        if not cliente or not vehiculo:
            raise HTTPException(status_code=500, detail="Error: datos inconsistentes en el alquiler")
    ...
    @staticmethod
    def cancelar_alquiler(alquiler_id: int) -> JSONResponse:
        alquiler = next((a for a in alquileres_db if a.id == alquiler_id), None)
        if not alquiler:
            raise HTTPException(status_code=404, detail="Alquiler no encontrado")
